...
- [ ] `recent_chats` 的执行路径对最近 30 个会话也不会调用完整节点加载，返回字段和排序保持不变。
- [ ] 节点同步通过轻量 ID 投影计算删除项，既有消息 JSON 不因 diff 被反序列化或复制。
- [ ] 新建/分叉会话的节点编码与插入按固定批次执行，测试证明空列表、小列表和大列表的批次边界。
- [ ] 完整会话与索引重建仍能还原全部节点、顺序、收藏状态和选择分支。
- [ ] 诊断日志只包含操作名、会话 ID、页/节点/字符计数、耗时和 heap 数值，不包含消息正文、工具参数或附件内容。
- [ ] ConversationRepository/ConversationTools 相关测试、app 编译和可用设备安装通过，或明确记录真实环境失败。
