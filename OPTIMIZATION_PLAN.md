...
|----|----------|------------------------|--------------------|------|------------|-----------------|
| B1 | P0 | Enable Gradle parallel execution; ten modules, setting disabled in `gradle.properties` | Overlap independent module compilation/resource/native tasks | Medium: RAM contention or coupled tasks | Same 3-sample cold protocol, tests, assemble | Measuring |
| B2 | P0 | Verify `web:buildWebUi`; wired to every `preBuild`, but baseline observed UP-TO-DATE | Avoid external pnpm work only if incrementality is broken | Medium: stale embedded UI | Two `:web:preBuild --info` runs and input-change check | Pending evidence |
...
| R1 | P1 | Cache `MarkdownWeb` HTML template; current function rereads asset per preview | Remove UI-path disk read and repeated invariant template allocation | Low/medium | Focused template-output test, StrictMode/trace proxy, app install | Pending |
...
| R4 | P1 | LRU key roulette reads/parses/writes cache under global lock per request | Reduce disk/JSON/lock overhead while preserving fairness | Medium: crash persistence/concurrency | Deterministic concurrency/persistence tests and benchmark | Pending evidence |
