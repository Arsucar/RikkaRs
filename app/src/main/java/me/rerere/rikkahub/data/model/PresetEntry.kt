...
/**
 * 预设条目 (见 issue #182 / #259)
 *
 * 将「预设 = ModeInjection ID 容器」升级为「可编辑、可开关、可排序的提示词条目」。
 * 两个变体：
 * - [Custom]: 自定义内容条目，内容内嵌于预设（快照语义）。
 ...
 *
 * #259: 原 Reference 变体已删除；升级时 Reference → Custom 内容快照。
 */
@Serializable
sealed class PresetEntry {
     ...
     * 自定义条目 - 内容内嵌
     *
     * @param legacyPriority 迁移快照专用：旧全局注入 priority 原值。
     *   非 null 时（由 [migratedWithEntries] 写入）注入解析用它参与全局混排排序；
     *   用户新建条目为 null，回退到 `-order`（见 #182）。
     */
    @Serializable
    @SerialName("custom")
    data class Custom(
...
/** 与 PresetDetailPage 的 Builtin / Custom 分区保持一致的稳定展示顺序。 */
